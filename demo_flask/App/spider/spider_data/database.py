...
class Database(object):
    def __init__(self, host, username, password, db_name):
        self.__connection = connect(host=host, user=username, password=password, port=10306, database=db_name)
        self.__cursor = self.__connection.cursor()

    def execute_sql(self, table_name, mode="search", select="*", data_type=None, **kwargs):
        """
        ...
        if mode == "search":
            if data_type:
                if kwargs['key'] == '弹幕' and data_type == 'all_list':
                    sql = "select {} from {}".format(kwargs['key'], table_name)               
                    search_list=[]
                    self.__cursor.execute(sql)
                    for item_search in self.__cursor.fetchall():
                        search_list.append(item_search)    
                    return search_list
                elif isinstance(kwargs['key'], str) and isinstance(kwargs['value'], str) and data_type == 'all_list' :
                    sql = "select {} from {} where {}='{}'".format(select, table_name, kwargs['key'], kwargs['value'])
                    search_list=[]
                    self.__cursor.execute(sql)
                    for item_search in self.__cursor.fetchone():
                        search_list.append(item_search)    
                    return search_list   
            else:                 
